...
/**
 * Creates the light rig used by the main artifact viewer (gallery 3D view).
 * Returns named references so applyLightingMode() / cycleLighting() can
 * adjust intensities for the gallery / daylight / dramatic modes.
 * @param {THREE.Scene} scene
 * @returns {object} named light references
 */
function createLightRig(scene){
  const lights = {};
  lights.hemi = new THREE.HemisphereLight(0xfff3df, 0x4a3218, 1.9);
 ...
 * Colors/intensities intentionally mirror createLightRig()'s warm gallery
 * palette (0xffe8c2 key / 0xC6A15B rim / 0xfff3df-0x4a3218 hemisphere) so the
 * homepage preview reads consistently with the main artifact viewer.
 */
